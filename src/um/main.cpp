...
int main() {
    ...
    BOOL loaded = Com::LoadDriverHandle(DRIVER_REGISTRY_PATH);
    if (!loaded) {
        ...
        if (!Com::LoadDriverHandle(DRIVER_REGISTRY_PATH)) {
    ...
    BOOL status = MM::ManualMapDLL(pid, pSrcData);
    if (!status) {
    ...
    FADED_RED("[+] DLL mapped into the memory of process successfully.\n");
    FADED_RED("[+] Terminating...\n");
